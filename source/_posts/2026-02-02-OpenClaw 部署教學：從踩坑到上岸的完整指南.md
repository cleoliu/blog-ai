...
OpenRouter 最近推出了 `openrouter/free` 這個特殊的路由器模型，完全免費使用。

- **免費方式**：註冊 [OpenRouter](https://openrouter.ai) 帳號，取得 API Key
- **優點**：
  - 完全免費（input/output 都是 $0）
  - 智慧路由：根據你的請求需求（圖片理解、tool calling、structured outputs）自動選擇支援該功能的免費模型
  - 支援多模態輸入（text + image），最高 200k context
  - 無需手動追蹤哪些模型目前免費
- **缺點**：每次請求可能路由到不同模型，回覆風格會有差異
- **目前可能路由到的免費模型**：
  - StepFun Step 3.5 Flash（196B MoE，256k context，推理模型）
  - Arcee AI Trinity Large Preview（400B MoE，131k context）
  - Upstage Solar Pro 3（102B MoE，128k context，韓文優化）
  - LiquidAI LFM 2.5 系列（輕量級，適合邊緣裝置）
  - 以及其他提供免費額度的模型（會動態更新）
- **設定方式**：
  1. 去 [OpenRouter](https://openrouter.ai) 註冊並取得 API Key
  2. 執行 `openclaw models auth --provider openrouter --mode apiKey`
  3. 在 Config 設定模型為 `openrouter/openrouter/free`
...
如果你想把 OpenAI Codex 設為主要模型，可以照以下步驟：

**適合使用情境**：
- 程式碼相關任務（除錯、重構、理解大型專案）
- 需要長上下文（長文件/規格）
- 需要穩定、可預期回覆品質的場景

```bash
# 1) 進行 OpenAI Codex 授權（會開啟瀏覽器 OAuth）
...
Configured models (4): google-antigravity/claude-opus-4-5-thinking, qwen-portal/coder-model, qwen-portal/vision-model, openai-codex/gpt-5.2-codex
```

> 如果你不想把它設成預設模型，可以省略第 2 步，之後用 `/model openai-codex/gpt-5.2-codex` 或 `openclaw models set ...` 切換即可。
