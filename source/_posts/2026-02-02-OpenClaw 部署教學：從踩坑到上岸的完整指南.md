...
- **Qwen Code / Qwen Portal OAuth**：
  - 官方頁面寫明 **2,000 free requests/day**。
  - 未標示 token 上限與重置時間（官方未公開）。
  - 來源：Qwen Code Docs
  - https://qwenlm.github.io/qwen-code-docs/
- **OpenAI Codex（ChatGPT Plus OAuth）**：
  - 官方僅提到 Plus/Pro 的 Codex 有更高的 rate limits，但**沒有公開固定 token/次數**。
  - 來源：OpenAI Help Center（Codex app release notes）
  - https://help.openai.com/en/articles/6825453-chatgpt-usage-limits
- **Google Antigravity**：
  - 目前未找到官方公開的免費額度數字（token/次數/重置時間）。
...
- **免費額度**：屬於平台提供的試用/免費配額，**會重置且可能隨時調整**
- **限制/風險**：免費試用期與配額可能隨時調整，甚至封鎖 API 轉發
...
- **設定方式**：
  1. 訪問 https://portal.qwen.ai 建立帳號
  2. 執行以下指令（會開啟瀏覽器 OAuth）：
...
- **補充**：本文作者是 **ChatGPT Plus 訂閱者**，使用 `openai-codex` OAuth 授權
- **設定方式**：
