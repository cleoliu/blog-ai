...
以下是 Cat-Raising 應用的主要界面截圖：

![首頁界面](https://res.cloudinary.com/dsvl326mi/image/upload/c_fill,w_800,h_800,g_center/v1770293992/blog_covers/c2egznglswshssppfxxd.png)

*首頁展示了玻璃質感設計和漸變色彩搭配*

*[註：以下截圖為示意圖，實際應用包含以下功能界面]*

### 主要功能界面

**1. 登入/註冊頁面**
支持一般帳號註冊和 Google OAuth 登入，採用玻璃質感卡片設計，提供流暢的用戶體驗。

**2. 營養計算頁面**
直觀的表單設計，支持輸入品牌名稱、食品名稱、重量及詳細營養成分（蛋白質、脂肪、纖維、灰分、水分、碳水化合物、礦物質等），並即時顯示計算結果。

**3. 結果展示頁面** 
以視覺化方式呈現營養分析結果，使用綠色/紅色標示營養成分是否達到 AAFCO 標準，包含乾物質分析、熱量比分析等。

**4. 貓咪管理頁面**
可建立多隻貓咪檔案，提供 16 款可愛貓咪頭像選擇，記錄姓名、生日、體重等基本資料。

**5. 計算記錄頁面**
以卡片形式展示歷史計算記錄，支援收藏功能和時間排序，方便用戶追蹤和比較不同食品的營養價值。

**6. 手機響應式設計**
針對手機使用優化，採用底部導航設計，完美適配各種尺寸的行動設備，並支援 PWA 安裝功能。
